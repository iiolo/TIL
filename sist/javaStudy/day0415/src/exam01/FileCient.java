	...
	public void actionPerformed(ActionEvent e) {
		...
		if(cmd.equals("파일선택")) {
			...
			if(re == JFileChooser.APPROVE_OPTION) {
				...
				//전송하기 위하여 선택한 파일을 맴버변수 file에 담아둡니다.
				file = jfc.getSelectedFile();
				
				//선택한 파일이름을 텍스트필드에 출력합니다.
				jtf.setText(file.getName());
			}
		}else if(cmd.equals("파일전송")) {
			try {
				//선택한 파일을 메모리로 읽어들기 위한 FileInputStream객체를 생성합니다.
				FileInputStream fis = new FileInputStream(file);
				
				//파일의 내용을 읽어들이기 위한 배열을 만들어요
				byte []data = new byte[200];
				
				//파일의 끝이 아닐때까지 읽어들입니다.
				while(  fis.read(data) != -1  ) {
					
					//읽어들인 내용을 스트림을 통해 서버로 보냅니다.
					os.write(data);
					
					//다음데이터를 위하여 배열을 비워줍니다.
