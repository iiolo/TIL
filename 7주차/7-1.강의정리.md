...
- 프로토콜 문제 해결 : 이 프로토콜 때문에 얼마나 에먹었는지 모른다. 진짜 검색 또 검색을 통해서 해결했다. 일단 http는 포트번호를 어떻게 해야할 지를 모르겠어서 ssh로 해결을 봤다. ssh를 발급받은 다음 그걸 github에 등록하고 소스트리와 gitbash에서 사용할 수 있도록 했다. 컴퓨터 내 폴더를 몇번이나 지웠다가 다시 만들었다. 그래도 해결이 되서 다행이다. 
