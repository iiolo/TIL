...
public class DictionaryClientUDP {
	public static void main(String[] args) {
		try {
			...
			//키보드 입력을 위한 스캐너객체를 생성합니다.
			Scanner sc = new Scanner(System.in);
			
			//사용자로 부터 영어단어 입력을 위한 변수를 선언
			String eng;
			
			//서버로 부터 번역되어 수신된 한국어를 저장하기 위한 변수를 선언
			String kor;
			
			//UDP방식의 데이터 전송단위인 데이터그램패킷 변수를 선언합니다.
			...
			//서버로 부터 번역된 한국어를 수신하기 위한 데이터를 받을 패킷을 생성합니다.
			DatagramPacket rePacket = new DatagramPacket(data, data.length);
						
			//패킷을 생성할 때에 필요한 InetAddress객체를 생성합니다.
			InetAddress addr = InetAddress.getByName("211.238.142.98");
			
			//패킷을 생성할 때에 필요한 포트번호를 변수에 저장합니다.
			int port = 9004;
			
			//계속 반복하여 실행합니다.
			while(true) {
				
				//사용자한테 번역할 영어단어를 입력받아요
				System.out.print("번역할 영어단어를 입력==>");
				eng = sc.next();
				...
				//번역한 한국어를 출력합니다.
				System.out.println("서버로 부터 번역하여 수신한 데이터 : " + kor);
